                            ...
                            <tbody>
                                @foreach ($users as $user)
                                    <tr wire:key="{{ $user->id }}" class="border-b dark:border-gray-700">
                                        <th scope="row" wire:click="setSortBy('name')"
                                            class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ $user->name }}</th>
                                        <td wire:click="setSortBy('email')" class="px-4 py-3">{{ $user->email }}</td>
                                        <td wire:click="setSortBy('is_admin')"
                                            class="px-4 py-3  {{ $user->is_admin ? 'text-green-500' : 'text-blue-500' }} ">
                                            {{ $user->is_admin ? 'Admin' : 'Member' }}</td>
                                        <td wire:click="setSortBy('created_at')" class="px-4 py-3">
                                            {{ $user->created_at }}</td>
                    ...
                    <div class="py-4 px-3">
                        <div class="flex ">
                            <div class="flex space-x-4 items-center mb-3">
                                ...
                                <select {{-- start for showing pagination livewire --}} wire:model.live='perPage' {{-- end for showing pagination livewire --}}
                                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
